...
canEventuallyContainShinyGold rules bag = isJust (lookup shinyGold rulesForBag) 
    ...
    where
        rulesForBag = concatMap bagRules $ filter ((==bag).outerBag) rules

countSubBags :: Bag -> [Rule] -> Integer
countSubBags bag rules = sum [n * (1 + countSubBags innerBag rules) | (innerBag, n) <- rs]
    where
        (Rule _ rs) = head $ filter ((==bag).outerBag) rules
